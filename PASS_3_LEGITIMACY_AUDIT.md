...
| **GAME38** | CR3BP-Inspired Low-Energy Transit Corridor Approximation | `src/simulation/interplanetary-highway.ts` | Working model | **NARROWED** | Accurately narrowed across code, UI, tests, and documentation: Approximates low-energy transit corridors around planar circular restricted three-body problem (CR3BP) L1/L2 points without claiming numerical invariant manifold integration. |
...
| **GAME45** | Oort-Cloud Long-Period Comet Injection Generator | `src/simulation/oort-comets.ts` | Working generator | **NARROWED** | Removed unsupported galactic-tide claim; accurately named long-period comet injection generator providing near-parabolic ($e \sim 0.985 - 0.999$), highly inclined Keplerian state vectors with deterministic parameter control. |
...
| ID | Title | File Location | Initial State | Audit Verdict | Repair & Integration Performed |
|---|---|---|---|---|---|
| **BACK31** | Step-Doubling Adaptive Velocity Verlet Timestep Controller | `src/simulation/adaptive-timestep.ts` | Standalone class | **REPAIRED** | Re-engineered as true step-doubling Velocity Verlet controller using Richardson extrapolation local error estimation. Symplectic fixed-step sub-stepping preserved as default; adaptive error control optionally enabled in `simulation/engine.ts`. |
...
| **BACK44** | Web Worker Computation Thread Pool & Integrator Bridge | `src/core/thread-pool.ts` | Working thread pool | **REPAIRED** | Integrated into production UI via `WorkerIntegratorBridge` in `EphemerisExportModal.tsx`, exported on `window.__starsilk_worker_pool__` in `App.tsx`, and verified via live CDP worker execution in `scripts/android-adb-qa.mjs`. |
